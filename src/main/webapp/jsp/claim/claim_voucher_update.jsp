		...
		<div class="pages">
			...
			<input type="hidden" id="claimId" name="claimVoucher.id" value="<s:property value="claimVoucher.id"/>"/>
				<table width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
                  ...
                  <tr>
                    <td>总金额：
                    <s:textfield name="claimVoucher.totalAccount" id="totalAccount" readonly="true"  theme="simple"/></td>
                    <td>填报时间：<s:date name="claimVoucher.createTime" format="yyyy-MM-dd HH:mm:ss"/></td>
                    <td>状态：<s:property value="claimVoucher.status"/></td>
                    <td>待处理人：<s:select id="deal" name="claimVoucher.nextDeal.sn" list="#session.manager" listKey="sn" listValue="name" headerKey="" headerValue="--请选择--" theme="simple"></s:select>
                    <s:hidden id="status" name="claimVoucher.status" value="%{claimVoucher.status}"  theme="simple"/>
                  </tr>
                  <tr>
                  	<td colspan="4"><p>-------------------------------------------------------------------------------</p></td>
                  </tr>
				...
				<table id="myTable" width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
					...
					<tr>
						<td>
							<s:hidden id="id%{#s.index}" name="detailList[%{#s.index}].id" value="%{#claimDetail.id}"  theme="simple"/>
							<s:hidden id="item%{#s.index}" name="detailList[%{#s.index}].item" value="%{#claimDetail.item}"  theme="simple"/>
							<s:property value="#claimDetail.item" /></td>
						<td>
							<s:hidden id="account%{#s.index}" name="detailList[%{#s.index}].account" value="%{#claimDetail.account}"  theme="simple"/>
							<s:property value="#claimDetail.account" /></td>
						<td>
							<s:hidden id="desc%{#s.index}" name="detailList[%{#s.index}].desc" value="%{#claimDetail.desc}"  theme="simple"/>
